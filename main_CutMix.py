...
# Training
writer = SummaryWriter('./log/cutmix')
...
def pic_show(a,b):
    ...
    # 原图像列表
    original_images = pic_a[:3]

    # 变换图像列表
    transformed_images = pic_b[:3]

    # 标签列表
    labels = ["Transform 1", "Transform 2", "Transform 3", "Transform 4", "Transform 5", "Transform 6"]
    ...
    for i in range(3):
        ...
        # 显示变换图像
        axes[i, 1].imshow(np.transpose(transformed_images[i], (1, 2, 0)))
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/cutmix_ckpt.pth')
        best_acc = acc
